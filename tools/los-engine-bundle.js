...
function nickLineOfSight(fromTileX, fromTileY, toTileX, toTileY, state) {
  ...
  // Extension: optional debug callback. Called once per corner-pair attempt
  // with `(attackerCorner, defenderPair, result)` so a UI can paint each
  // attempted line with its individual outcome.
  const debugCornerPair        = state.debugCornerPair        || null;
    ...
    for (const [d1, d2, mid] of defenderPairs) {
      ...
      if (debugCornerPair) {
        debugCornerPair({
          ...
          defender2: d2,
          midpoint: mid,
          overlaps, c1: !!c1, c2: !!c2, m: !!m, open,
        });
      }
      if (open && !anyOpen) {
        anyOpen = true;
        if (!debugCornerPair) return true;  // fast path when no debug listener
      }
    }
  }
  return anyOpen;
