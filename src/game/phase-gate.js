...
/**
 * Walk game state to determine who the game is waiting on.
...
export function getWaitingPlayers(game) {
  ...
  // 2. pendingCombat — not both ready
  ...
  // 8. CC draw phase (hand channels exist, not drawn)
  if ((game.p1HandId || game.p2HandId) && (!game.player1CcDrawn || !game.player2CcDrawn)) {
    // Only if deployment is done
  ...
  // 14. currentActivationTurnPlayerId
