 ...
 *        is this ability eligible in the current combat state?
...
export function registerCombatAbility(entry) {
  ...
  if (typeof applies !== 'function') throw new Error(`registerCombatAbility(${id}): applies must be a function`);
  _registry.set(id, { id, name, windows: [...windows], side, kind, applies, special: entry.special || null });
}
