...
export async function getMapAttachmentForSpaces(game, validSpaces, deps) {
  ...
  try {
    ...
    const tokens = deps.getMapTokensForRender(map.id, game?.selectedMission?.variant, game?.openedDoors, game?.ancillaryTokens, game?.selectedMission?.tokenLabel || 'Token');
