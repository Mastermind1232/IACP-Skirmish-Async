...
export async function deleteGameChannelsAndGame(game, gameId, ctx) {
  ...
  if (categoryId) {
    try {
      ...
      if (guild) {
        ...
        await guild.channels.fetch(categoryId).then((cat) => cat?.delete()).catch(() => {});
      }
    } catch (err) {
  ...
  deleteGame(gameId);
  saveGames();
  if (deleteGameFromDb) await deleteGameFromDb(gameId).catch(() => {});
...
export async function handleBotmenuArchive(interaction, ctx) {
  ...
  if (!game) {
    await interaction.reply({ content: 'Game not found or already deleted.', ephemeral: true }).catch(() => {});
    return;
  }
  if (interaction.user.id !== game.player1Id && interaction.user.id !== game.player2Id) {
    await interaction.reply({ content: 'Only players in this game can archive it.', ephemeral: true }).catch(() => {});
    return;
  }
...
export async function handleBotmenuArchiveYes(interaction, ctx) {
  ...
  try {
    ...
    await interaction.editReply({ content: `Game **IA Game #${gameId}** archived. Channels removed.` }).catch(() => {});
  } catch (err) {
...
export async function handleBotmenuKillYes(interaction, ctx) {
  ...
  if (!canKillGame(interaction, game)) {
    await interaction.reply({ content: 'You are not allowed to kill this game.', ephemeral: true }).catch(() => {});
    return;
  }
  await interaction.deferReply({ ephemeral: true });
  try {
    await deleteGameChannelsAndGame(game, gameId, ctx);
    await interaction.editReply({ content: `Game **IA Game #${gameId}** deleted. All channels removed.` }).catch(() => {});
  } catch (err) {
