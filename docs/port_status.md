...
| Combat resolution | `src/handlers/combat.js` (6335 LOC) + `src/engine/combat-bridge.js` (2930 LOC) + `src/game/combat.js` (370) | `python/engine/mechanics/combat.py` (248), `combat_declare.py`, `combat_defense_friends.py`, `attack_orchestrator.py` (662) | CLOSE | **Core combat math (`compute_combat_result`) verified byte-identical with JS `computeCombatResult` across 2011 fuzz cases + 11 edge cases. Pytest regression.** Attack orchestration (figure lookup, condition consumption, damage application, trigger firing, defeat) still needs end-to-end parity. Commit c0c5189. |
