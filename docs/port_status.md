...
| Legal-action enumerator | `src/engine/available-actions.js` (2394 LOC) | `python/mcts/actions.py` | PARTIAL | Harness at `python/parity/legal_actions_golden.py`. 30-case fuzz: 19 PASS, 11 FAIL on enumerator-philosophy gaps (JS 2-step MOVE_FIGURE → MOVE_PICK_SPACE vs Python 1-step; game-over button semantics). No rule divergences, just shape differences. Python is MCTS-accurate; JS is Discord-UI-oriented. Commit 3ccf019. |
| Combat resolution | `src/handlers/combat.js` (6335 LOC) + `src/engine/combat-bridge.js` (2930 LOC) + `src/game/combat.js` (370) | `python/engine/mechanics/combat.py` (248), `combat_declare.py`, `combat_defense_friends.py`, `attack_orchestrator.py` (662) | CLOSE | Core math `compute_combat_result`: byte-parity with JS across 2011 fuzz cases (commit c0c5189). Primitives (reduce_hp, heal_hp, apply/filter_condition): 400 fuzz cases (commit cd4016a). **Orchestrator sequencing**: 9 rules-oracle scenarios (hit, pendingCombat-clear, attack-log, defeat+VP+group wipe, bonus-conditions, friendly-fire reject) pass (commit 36af919). |
| Movement | `src/game/movement.js` (1099) + `src/handlers/movement.js` (1463) + `src/game/movement-interrupts.js` (200) | `python/engine/mechanics/movement_board.py` + `movement_cache.py` (501) + `interrupts.py` (237) | CLOSE | Basic MP/path cache done. Post-move interrupts (Parting Blow / Dirty Trick / Disengage / Overwatch) parity-verified across 6 scenarios — key-type bug (string vs int player-num) fixed. Commit 9597ea4. Remaining: figure rotation, strain-for-MP interactions unverified. |
...
| Round effects | `src/handlers/round.js` (2154) | `python/engine/mechanics/round_effects.py` (183), `stepper._handle_end_end_of_round` | CLOSE | EoR now fires: DC passives (Brush, Regenerate Bossk), mission EoR rules (14 types), Krykna activation (Chopper Base A), correct CC draw formula (1 + terminals + RHC - CtF), DC readying (movementBank/dcActionsData/exhaustedSkirmishUpgrades cleared), round-scoped state clear. Commits 3236868 + c677341. |
| Mission scoring | `src/game/mission-rules.js` (711) + `src/engine/mission-helpers.js` | `python/engine/mechanics/mission_rules.py` (979) | CLOSE | All 14 EoR rule types ported + wired into stepper at c677341. Per-mission end-to-end parity still un-verified (harness pending). |
