...
**Still in index.js (extract in follow-up):** Combat (attack_target_, combat_ready_, combat_roll_, combat_surge_) — note: `resolveCombatAfterRolls` is an async function defined inside the interaction handler; move it to top-level in index and pass in context, or move it into combat.js with full deps. Activation (status_phase_, pass_activation_turn_, end_turn_, confirm_activate_, cancel_activate_). DC-play-area (dc_activate_, dc_unactivate_, dc_toggle_, dc_deplete_, dc_cc_special_, dc_move_/dc_attack_/dc_interact_/dc_special_). Setup (map_selection_, draft_random_, determine_initiative_, deployment_zone_*, deployment_fig_, deployment_orient_, deploy_pick_, deployment_done_, deploy_modal_). CC-hand (squad_modal_, deploy_modal_, cc_attach_to_, cc_play_select_, cc_discard_select_, deck_illegal_*, cc_shuffle_draw_, cc_play_, cc_draw_, cc_search_discard_, cc_close_discard_, cc_discard_, squad_select_). Same pattern: create handler file, register, build context in index, replace if-block.
