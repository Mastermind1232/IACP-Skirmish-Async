...
When you ask to **fully wire** a card, that means the bot performs the effect (game state + any needed player choices). Some cards need only backend work (`abilities.js` + game state). Others need **intuitive Discord implementation** too: choice UIs, map interaction, or reactive prompts. Existing patterns:

- **Simple choice:** `requiresChoice` + `choiceOptions` (array of strings) → handler shows buttons (cc-hand.js, dc-play-area.js); player picks, then `resolveAbility(id, { ... context, choiceIndex })`. Good for "choose one of these".
- **Targeted choice:** `choiceTarget` (e.g. `readyAdjacentFriendlyDeploymentCard`) → handler must provide the chosen value in context (e.g. which DC name). Today only some flows pass that; others may still be manual.
- **Pick a space:** Movement already does this: **zoomed-in map image + grid overlay** in the thread via `getMovementMinimapAttachment` + `getMoveSpaceGridRows` (buttons per valid space). We can reuse that pattern for any CC that needs "choose a cell": compute valid spaces (e.g. empty adjacent), send same style message (space buttons + optional minimap cropped to those spaces), then a handler that completes the CC when they click a space (e.g. `cc_space_${gameId}_${abilityId}_${space}`).
- **Reactive (e.g. cancel opponent card):** Requires hooking into the moment the opponent plays a card and prompting the reacting player. New flow.

**Cards that need more than backend — why not implemented, and how we could:**

| Card | Why it’s not implemented | How we could implement it |
|------|---------------------------|----------------------------|
| **Behind Enemy Lines** | Needs deck read (top 3 of opponent) + reorder. No "show opponent deck" or "reorder deck" UI. | Add: read opponent deck top 3, show as choice labels (e.g. "1st, 2nd, 3rd" = card names); buttons to set order (e.g. "Put X on top") or a second step with 3 options "Top / Middle / Bottom" per card. Backend: reorder deck array. |
| **Blaze of Glory** | "Ready one of *your* DCs" needs a list of your DCs as buttons; "at EOR suffer 3 Damage" needs round-end hook. | **Ready DC:** Same as New Orders but for *your* DCs: return `requiresChoice` + `choiceOptions` = list of your DC display names (from game.p1DcList / p2DcList), handler already supports choiceOptions → buttons; on pick, pass `readyOwnDeploymentCardDcName` (or similar) in context and add a branch that readies that DC. **EOR damage:** Store `game.endOfRoundSelfDamage[playerNum] = 3` (or by figure); in round.js end-of-round flow, apply that damage then clear. |
| **Collect Intel** | No game state to change; "look at hand" is just visibility. | Already fine as message-only: post reminder "Look at opponent's Hand channel." No extra UI. Can mark as wired if we accept "message = resolved". |
| **Comm Disruption** | Reactive: must fire when opponent plays a cost-X card, before it resolves. | In CC play handler (cc-hand.js): when the *other* player is the one playing the card, check if this player has a Comm Disruption–style card and SPY count ≥ cost; if so, send them a prompt "Cancel this play? [Yes] [No]". If Yes, don’t apply the played card and remove it from their hand / return to hand. Backend: abort the resolveAbility for that card. |
| **Coordinated Attack** | Two attacks (you + friendly), same target. Combat flow is single-attack. | Option A: Run combat twice in sequence (first attack resolves, then same target for second attack from the chosen friendly). Option B: Add a "second attacker" mode where after first combat we prompt for second declaration with same target. Needs combat handler to support "second attack, same target" and which figure is attacking. |
| **Data Theft** | Need to show opponent’s discard and let player pick one; then allow playing that card as theirs this round. | **Pick from discard:** `choiceOptions` = opponent’s discard list (e.g. card names); handler already does buttons. Store `game.stolenCcThisRound[playerNum] = chosenCard`. When they "play" a card this round, if it’s the stolen one, resolve from opponent’s discard (move to their hand for the play, then remove from opponent discard). Backend + small handler change to allow playing from "stolen" slot. |
| **Cloned Reinforcements** | Place defeated figure on map: which figure + which empty adjacent space. | **Pick figure:** `choiceOptions` = list of defeated friendly non-DROID non-unique TROOPER cost ≤4 (from game state of defeated figures). **Pick space:** Reuse **pick a space** like movement: valid spaces = empty adjacent to activating figure(s); send grid buttons + zoomed map (`getMoveSpaceGridRows`-style with prefix `cc_place_`, plus map attachment for those spaces). Handler: on space pick, set figure position to that space, mark figure as no longer defeated (or restore to board). Backend: figure placement state. |
| **Endless Reserves** | Same as Cloned Reinforcements but different filter (defeated TROOPER, adjacent to same group). | Same pattern: choiceOptions for which defeated TROOPER, then **pick a space** (empty adjacent to same group) via movement-style grid + map. Backend: placement. |
| **New Orders** (partial) | `choiceTarget: readyAdjacentFriendlyDeploymentCard` but handler never supplies `readyAdjacentFriendlyDcName`. | In dc-play-area (or wherever CC is played from DC): when result has `choiceTarget === 'readyAdjacentFriendlyDeploymentCard'`, compute adjacent friendly figures (from board state), build `choiceOptions` = their DC names, send buttons; on pick, call resolveAbility again with `readyAdjacentFriendlyDcName: chosen`. So: add one branch in the handler that turns choiceTarget into choiceOptions when we can compute the list. |
| **Change of Plans** | Exhaust 1 DC, ready 1 other (same trait). Two sequential choices from your DCs. | First choice: buttons = your DCs (which to exhaust). Second choice: buttons = your other DCs with same trait (which to ready). Handler: after first pick, set pending state and send second button set; on second pick, apply both. Backend: exhaustOneDeploymentCard + readyOneDeploymentCard (by DC name). |
| **Force Push** | Choose SMALL figure within 3, then push 2. Figure picker + push destination. | **Figure:** choiceOptions = figures in range (SMALL, within 3) — need a helper to list them; then buttons. **Push destination:** Reuse **pick a space**: valid spaces = up to 2 from that figure along a push direction (or "choose direction" then "choose distance"). Could be two-step: direction buttons (N/S/E/W or adjacent cells), then space grid for valid landing. Movement already has "valid spaces"; we need "valid push destinations from X". |
| **Ballistics Matrix / Marksman** | "Figures don’t block LOS" — no player choice. | Backend only: add a flag (e.g. on combat or on game) that LOS check reads so it ignores blocking figures for this attack. No Discord UI. |

**Reusable building block:** **Pick a space** = same as movement: `getMoveSpaceGridRows` (or a generic `getSpaceChoiceRows(customIdPrefix, validSpaces, mapSpaces)`) plus a map attachment (e.g. `getMovementMinimapAttachment`-style cropped to `validSpaces`). Handler for `cc_space_*` or `cc_place_*` completes the CC with the chosen coord. So placement (Cloned Reinforcements, Endless Reserves) and push destination (Force Push) can all use the same "zoomed grid + buttons" pattern we already have for movement.
