...
| F6 | Conditions and combat extras | ✅ **Done (core).** Conditions: stun/weaken/bleed applied to defender and stored in `game.figureConditions[figureKey]`. **Conditions displayed** in DC embed health section (formatHealthSection + getConditionsForDcMessage). Recover: parsed and applied (heal attacker by N after combat). Blast: parsed and applied (figures adjacent to target suffer N damage via `getFiguresAdjacentToTarget`). Cleave: parsed in surge (surgeCleave); applying cleave requires extra target selection (UI) — deferred. |
