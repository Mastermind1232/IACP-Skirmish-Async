...
| F11 | Post-game archival | **Archive** is reached via the **slash command** `/botmenu` in the Game Log that opens the Bot Stuff menu → Archive → "Are you sure you want to archive?" Yes/No. Entry point is **hidden** (no prominent button) so players don't accidentally trigger destructive actions. Either player can use Archive. **First confirm wins** — whoever chose Archive can click Yes and the game is archived immediately; the other player cannot veto. On confirm: **delete** the game's Discord category and channels (game disappears from the server). If the game **had ended**, a row was already written to `completed_games` when the game ended (DB2); Archive just removes the game from the `games` table. If the game had **not** ended: no row in completed_games; remove from `games` and it vanishes. |
| F12 | Mission B (and other IACP missions) | Verify Mission B (contraband retrieve/deliver, VP) and any other mission variants end-to-end; fix or implement missing steps. Current Discord layout is sufficient (e.g. pinned mission card); no mission-specific channels or structure. |
...
| F15 | Discord limits and UI | Follow section 2.5 in every handler: max 5 buttons per row, 5 rows, 80-char labels, rate limits; use centralized discord/* helpers and color-by-area scheme. |
| F16 | Permissions and flow for destructive actions | **Kill Game** and **Archive** live under **Bot Stuff**, opened via slash command **`/botmenu`** — **hidden** so players don't accidentally kill or archive. `/botmenu` works **only in the Game Log channel** for that game so both players can see it; the bot infers the game from the channel. If used in a channel that is not a game's Game Log, reply: "Use /botmenu in the Game Log channel of the game you want to manage." When implementing /botmenu, remove the visible Kill Game button from setup (Kill Game only via /botmenu). Flow: /botmenu → menu (Archive, Kill Game only for now; extend later if needed) → for destructive options, confirmation "Are you sure?" Yes/No. Kill Game confirmation same as Archive: first confirm wins (no veto). **Kill Game** = delete without archiving (delete Discord + remove from `games`; never write to completed_games). Restrict **Kill Game** to game participants or users with the Discord role **Admin** or **Bothelpers** (fixed role names the bot expects; not configurable). **Archive** to either player. Document in UX. |
| F17 | Map Selection menu | Replace single Map Selection button with a menu of four routes: **Random** (random from all missions), **Competitive** (random from current tournament rotation; rotation updated every few months per D4), **Select Draw** (player selects multiple missions from a dropdown list, then random draw from that subset; require at least 2 — if only 1 chosen, prompt to use Selection instead), **Selection** (player selects one mission from a dropdown list). Use dropdown(s) for list choices; stay within Discord limits (e.g. 25 options per menu, chunk/paginate if more). Rotation list configurable (D4). **Whoever clicks the Map Selection button first** chooses (for Select Draw and Selection). Map selection is **visible in the Game Log** as buttons/menu during setup — not behind /botmenu; only Archive and Kill Game are behind /botmenu. |
