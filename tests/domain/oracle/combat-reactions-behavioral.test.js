...
// ── B-CR-VI: Veteran Instincts ──────────────────────────────────────────────

describe('B-CR-VI: Veteran Instincts two-phase bonuses', () => {
  it('B-CR-VI-001: attack phase — +1 Hit increases attackRoll.dmg', async () => {
    const combat = makeCombat();
    const game = {
      gameId: 'g1',
      player1Id: 'player1',
    ...
    assert.strictEqual(combat.attackRoll.dmg, 7, 'dmg 6→7');
    assert.strictEqual(combat.vetInstinctsAttackApplied, true);
  });

  it('B-CR-VI-002: attack phase — +1 Surge increases attackRoll.surge', async () => {
    const combat = makeCombat();
    const game = {
      gameId: 'g1',
      player1Id: 'player1',
    ...
    const { ctx } = buildCtx(game);
    await handleVetInstincts(mockInteraction('vet_instincts_pick_g1_surge', 'player1'), ctx);

    assert.strictEqual(combat.attackRoll.surge, 3, 'surge 2→3');
    assert.strictEqual(combat.vetInstinctsAttackApplied, true);
  });

  it('B-CR-VI-003: defense phase — +1 Block increases defenseRoll.block', async () => {
    const combat = makeCombat();
    combat.vetInstinctsAttackApplied = true; // attack phase already done
    const game = {
      gameId: 'g1',
      player1Id: 'player1',
      player2Id: 'player2',
    ...
    assert.strictEqual(combat.defenseRoll.block, 4, 'block 3→4');
    assert.strictEqual(combat.vetInstinctsDefenseApplied, true);
  });

  it('B-CR-VI-004: defense phase — +1 Evade increases defenseRoll.evade', async () => {
    const combat = makeCombat();
    ...
    const { ctx } = buildCtx(game);
    await handleVetInstincts(mockInteraction('vet_instincts_pick_g1_evade', 'player2'), ctx);

    assert.strictEqual(combat.defenseRoll.evade, 2, 'evade 1→2');
    assert.strictEqual(combat.vetInstinctsDefenseApplied, true);
  });

  it('B-CR-VI-005: skip attack phase sets vetInstinctsAttackApplied', async () => {
    const combat = makeCombat();
    const game = {
      gameId: 'g1',
      player1Id: 'player1',
    ...
    const { ctx } = buildCtx(game);
    await handleVetInstincts(mockInteraction('vet_instincts_pick_g1_skip', 'player1'), ctx);

    assert.strictEqual(combat.vetInstinctsAttackApplied, true, 'attack skip sets flag');
    assert.strictEqual(combat.vetInstinctsDefenseApplied, undefined, 'defense not yet applied');
  });

  it('B-CR-VI-006: defense phase enters reroll window with stored pending counts', async () => {
    const combat = makeCombat({
      attackerRerollsRemaining: 0,
      defenderRerollsRemaining: 0,
    });
    ...
    assert.strictEqual(combat.attackerRerollsRemaining, 1, 'atkRem restored from viPendingAtkRerolls');
  });

  it('B-CR-VI-007: defense phase proceeds when no rerolls pending', async () => {
    const combat = makeCombat();
    combat.vetInstinctsAttackApplied = true;
  ...
  it('B-CR-VI-008: defense phase enters forced reroll when forcedRerollQueue present', async () => {
    const combat = makeCombat();
    combat.vetInstinctsAttackApplied = true;
    combat.viPendingAtkRerolls = 0;
    combat.viPendingDefRerolls = 0;
    combat.forcedRerollQueue = [{ controlPlayer: 2, pool: 'attack', remaining: 1, source: 'Doubt' }];
    const game = {
      gameId: 'g1',
      player1Id: 'player1',
      player2Id: 'player2',
