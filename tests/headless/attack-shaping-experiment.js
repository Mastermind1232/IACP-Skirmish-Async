/**
 * Movement Shaping Reduction Experiment — Flat-only, 500 train + 50 eval.
 *
 * Tests whether halving movement shaping lets VP signal dominate.
 * ATTACK_SHAPE_REWARD = 0.50 (kept from v6).
 * Movement shaping halved: closing 0.075, engage 0.25, decision 0.15.
 * Eval uses argmax (not softmax).
 *
 * Baseline comparisons:
 *   v5 flat (no shaping, argmax):        36.3 atk/g, 1.2 VP
 *   v6 attack-shaping (argmax):          17.3 atk/g, 4.3 VP
 *   v7 attack-shaping + softmax τ=0.3:  64.1 atk/g, 2.6 VP
 *
 * Decision rule:
 *   CONFIRMED: attacks rise AND VP >= 3.0 → movement shaping was drowning VP
 *   DEEPER:    attacks stay low OR VP collapses → issue beyond shaping ratios
 *
 * Usage: node tests/headless/attack-shaping-experiment.js
 */
...
import {
  ...
  initializeNetwork, setGreedyMode, setSoftmaxEval,
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║  Movement Shaping Reduction Experiment (v8)              ║');
  ...
  console.log('║  500 training + 50 greedy eval                           ║');
  console.log('╚══════════════════════════════════════════════════════════╝');
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`Training: FLAT + ATTACK_SHAPING, ${TRAIN_GAMES} games`);
  ...
  for (let i = 0; i < EVAL_GAMES; i++) {
    const gameNum = TRAIN_GAMES + i; // different matchups
  ...
  // No-progress: rounds where total VP didn't increase
  ...
  console.log('\n' + '='.repeat(60));
  console.log('MOVE SHAPING REDUCTION RESULTS (v8)');
  console.log('='.repeat(60));
  console.log('');
  console.log('                       TRAIN (500)    v8 EVAL      v6(atk shp)  v5(baseline)');
  console.log(`  VP/game:             ${trainAvgVP.toFixed(1).padEnd(15)}${avgVP.toFixed(1).padEnd(13)}${'4.3'.padEnd(13)}1.2`);
  console.log(`  VP/round:            ${'-'.padEnd(15)}${vpPerRound.padEnd(13)}${'-'.padEnd(13)}-`);
  console.log(`  Attacks/game:        ${trainAvgAtk.toFixed(1).padEnd(15)}${avgAtk.toFixed(1).padEnd(13)}${'17.3'.padEnd(13)}36.3`);
  console.log(`  Moves/game:          ${(results.reduce((s, r) => s + r.moveActions, 0) / results.length).toFixed(1).padEnd(15)}${avgMoves.toFixed(1).padEnd(13)}${'200.1'.padEnd(13)}137.2`);
  console.log(`  Win rate:            ${((results.filter(r => r.winnerLabel).length / results.length) * 100).toFixed(1)}%${' '.repeat(11)}${((p1Wins + p2Wins) / n * 100).toFixed(1)}%${' '.repeat(9)}${'80.0%'.padEnd(13)}74.0%`);
  console.log(`  Avg rounds:          ${(results.reduce((s, r) => s + r.finalRound, 0) / results.length).toFixed(1).padEnd(15)}${avgRound.toFixed(1).padEnd(13)}${'9.4'.padEnd(13)}9.5`);
  console.log(`  No-progress rate:    ${'-'.padEnd(15)}${noProgressRate}%${' '.repeat(10 - noProgressRate.length)}${'88.7%'.padEnd(13)}-`);
  console.log(`  Surge spend rate:    ${'-'.padEnd(15)}${surgeRate}%${' '.repeat(10 - surgeRate.length)}${'N/A'.padEnd(13)}N/A`);
  ...
  if (rollback) {
    ...
    console.log('  >>> VERDICT: SUCCESS — attack shaping works, apply to graph next');
  } else {
  ...
  const checkpointPath = join(__dirname, 'learnings-data-moveshape-half-v1.json');
